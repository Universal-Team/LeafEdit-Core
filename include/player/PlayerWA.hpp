...
public:
	virtual ~PlayerWA() { };
	PlayerWA(std::shared_ptr<u8[]> playerData, u32 playerOffset, u8 index) :
	...
	std::unique_ptr<Item> islandbox(u8 slot) const override;
	std::unique_ptr<Item> storage(u16 slot) const override;
